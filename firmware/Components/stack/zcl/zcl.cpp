...
typedef struct zclCmdRecsList
{
  ...
  uint8                 endpoint;
  uint8                 numCommands;
  const zclCommandRec_t __code *pCmdRecs;
} zclCmdRecsList_t;
...
// The task Id of the Application where the unprocessed Foundation Command/Response messages will be sent to.
...
static ZStatus_t zclWriteAttrData( uint8 endpoint, afAddrType_t *srcAddr,  struct zclAttrRec_t *pAttr, ZclWriteRec *pWriteRec );
...
ZStatus_t zcl_registerPlugin( uint16 startClusterID, uint16 endClusterID, zclInHdlr_t pfnIncomingHdlr ){
	...
	pNewItem = (zclLibPlugin_t *)zcl_mem_alloc( sizeof( zclLibPlugin_t ) );
	if ( pNewItem == NULL ){
		return (ZMemError);
	}
...
ZStatus_t zcl_registerClusterOptionList( uint8 endpoint, uint8 numOption, zclOptionRec_t optionList[] )
{
  ...
  // Fill in the new profile list
  pNewItem = (zclClusterOptionList *)zcl_mem_alloc( sizeof( zclClusterOptionList ) );
  if ( pNewItem == NULL )
  {
    return (ZMemError);
  }
...
ZStatus_t zcl_SendCommand( uint8 srcEP, afAddrType_t *destAddr,  uint16 clusterID, uint8 cmd, uint8 specific, uint8 direction,  uint8 disableDefaultRsp, uint16 manuCode, uint8 seqNum, uint16 cmdFormatLen, uint8 *cmdFormat ){
...
#endif
  {
    ...
    // The cluster might not have been defined to use security but if this messageis in response to another message that was using APS security this message will be sent with APS security
  ...
  msgBuf = (unsigned char *)zcl_mem_alloc( msgLen );
  if ( msgBuf != NULL ) {
...
ZStatus_t zclSendCommand( uint8 srcEP, afAddrType_t *destAddr,  uint16 clusterID, uint8 cmd, uint8 specific, uint8 direction,  uint8 disableDefaultRsp, uint16 manuCode, uint8 seqNum, BufferData & bufferData){
  endPointDesc_t *epDesc;
  zclFrameHdr_t hdr;
  uint8 *msgBuf;
  uint16 msgLen;
  ...
  if ( msgBuf != NULL ) {
    ...
    // Fill in the command frame
    zcl_memcpy( pBuf, bufferData.data, bufferData.dataLen );
...
ZStatus_t zcl_SendRead( uint8 srcEP, afAddrType_t *dstAddr,  uint16 clusterID, zclReadCmd_t *readCmd,    uint8 direction, uint8 disableDefaultRsp, uint8 seqNum){
	...
	BufferData buf = readCmd->getBufferFromAttribute();
	if ( buf.isValid() ) {
		status = zclSendCommand( srcEP, dstAddr, clusterID, ZCL_CMD_READ, FALSE, direction, disableDefaultRsp, 0, seqNum, buf );
	} else {
		status = ZMemError;
	}
                           ...
                           uint8 direction, uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  buf = (unsigned char *)zcl_mem_alloc( len );
  if ( buf != NULL )
  {
 ...
 * @return  ZSuccess if OK
 */
ZStatus_t zcl_SendWriteRequest( uint8 srcEP, afAddrType_t *dstAddr, uint16 clusterID, ZclWriteCmd *writeCmd, uint8 cmd, uint8 direction,  uint8 disableDefaultRsp, uint8 seqNum ) {
	ZStatus_t status;

	BufferData bufferData = writeCmd->getBufferFromAttribute();
	if (bufferData.isValid()){
    	status = zclSendCommand( srcEP, dstAddr, clusterID, cmd, FALSE, direction, disableDefaultRsp, 0, seqNum, bufferData );
	} else{
		status = ZMemError;
 	}
	return status;
 ...
 * @return  ZSuccess if OK
 */
ZStatus_t zcl_SendWriteRsp( uint8 srcEP, afAddrType_t *dstAddr, uint16 clusterID, ZclWriteRspCmd *writeRspCmd, uint8 direction, uint8 disableDefaultRsp, uint8 seqNum ){
	ZStatus_t status;

	BufferData buffer = writeRspCmd->getBufferData();
	if (buffer.isValid()){
	    status = zclSendCommand( srcEP, dstAddr, clusterID, ZCL_CMD_WRITE_RSP, FALSE, direction, disableDefaultRsp, 0, seqNum,buffer );
	} else{
    	status = ZMemError;
  	}
                          ...
                          uint8 direction, uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  buf = (unsigned char *)zcl_mem_alloc( dataLen );
  if ( buf != NULL )
  {
    // Load the buffer - serially
    uint8 *pBuf = buf;
...
zclProcMsgStatus_t zcl_ProcessMessageMSG( afIncomingMSGPacket_t *pkt ){
  ...
  inMsg.pDataLen -= (uint16)(inMsg.pData - pkt->cmd.Data);
	...
	// Temporary workaround to allow callback functions access to the transaction sequence number.  Callback functions will call zcl_getParsedTransSeqNum() to retrieve this number.
    ...
    // But the Light Link cluster uses a different Frame Control format for it's Inter-PAN messages, where the messages could be confused with the foundation commands.
 ...
 */
static void *zclParseInWriteRspCmd( zclParseCmd_t *pCmd ){
	return new ZclWriteRspCmd(pCmd);
}
...
static uint8 zclProcessInReadCmd( zclIncoming_t *pInMsg ){
	...
	readRspCmd = (zclReadRspCmd_t *)zcl_mem_alloc( len );
	if ( readRspCmd == NULL ){
	...
	for ( i = 0; i < readCmd->numAttr; i++ ) {
		...
		if ( zclFindAttrRec( pInMsg->msg->endPoint, pInMsg->msg->clusterId, readCmd->attrID[i], &attrRec ) ){
			if ( zcl_AccessCtrlRead( attrRec.attr.accessControl ) ) {
				...
				if ( statusRec->status == ZCL_STATUS_SUCCESS ) {
					statusRec->data = (unsigned char *)attrRec.attr.dataPtr;
 ...
 */
static uint8 zclProcessInWriteCmd( zclIncoming_t *pInMsg ){
	ZclWriteCmd *writeCmd;
	ZclWriteRspCmd writeRspCmd;
	uint8 sendRsp = FALSE;
	uint8 j = 0;
	uint8 i;

	writeCmd = (ZclWriteCmd *)pInMsg->attrCmd;
	if ( pInMsg->hdr.commandID == ZCL_CMD_WRITE ){
		writeRspCmd = ZclWriteRspCmd(writeCmd);
		if (writeRspCmd.attrList == NULL){
			return FALSE;
		}
		sendRsp = TRUE;
	}

	ZclWriteRec *statusRec = writeCmd->attrList;
 ...
 * @return  none
 */
static void zclRevertWriteUndividedCmd(uint8 endpoint, uint16 clusterId,  ZclWriteRec *curWriteRec, uint16 numAttr, uint8 * oldData ){
 	uint8 i;

	struct zclAttrRec_t attrRec;
 	for ( i = 0; i < numAttr; i++ ){
		if ( !zclFindAttrRec( endpoint, clusterId,  curWriteRec->attrID, &attrRec ) ) {
      		break; // should never happen
    	}

		if ( attrRec.attr.dataPtr != NULL ){
			uint16 dataLen = zclGetAttrDataLength( attrRec.attr.dataType, curWriteRec->attrData );
      		zcl_memcpy( attrRec.attr.dataPtr, curWriteRec->attrData, dataLen );
    	}
		curWriteRec++;
	}
}
...
static uint8 zclProcessInWriteUndividedCmd( zclIncoming_t *pInMsg ){
	...
	ZclWriteRspCmd writeRspCmd(writeCmd);
	if (writeRspCmd.attrList == NULL){
		return FALSE;
	}
	...
	for ( i = 0; i < writeCmd->numAttr; i++ ) {
		...
		// Attribute Data length
		if ( attrRec.attr.dataPtr != NULL ) {
			dataLen = zclGetAttrDataLength( attrRec.attr.dataType, (unsigned char *) attrRec.attr.dataPtr );
		}
	...
	if ( writeRspCmd.numAttr ==  writeCmd->numAttr ){ // All attributes can be written
		...
		curWriteRec = (uint8 *) zcl_mem_alloc( curLen );
		if ( curWriteRec == NULL ){
			return FALSE; // EMBEDDED RETURN
    	}

		curDataPtr = curWriteRec;
		...
		for ( i = 0; i < writeCmd->numAttr; i++ ) {
			...
			if ( status != ZCL_STATUS_SUCCESS ) {
				// error, so a attribute status record are generated
				writeRspCmd.attrList[j].status = status;
				writeRspCmd.attrList[j++].attrID = statusRec->attrID;

				// Since this write failed, we need to revert all the pervious writes
				zclRevertWriteUndividedCmd(  pInMsg->msg->endPoint,  pInMsg->msg->clusterId,writeCmd->attrList, i, curWriteRec);
				break;
			}
    	...
    	writeRspCmd.numAttr = j;
    	if ( writeRspCmd.numAttr  ==  writeCmd->numAttr ) {
      		// Since all records were written successful, include a single status record in the resonse command with the status field set to SUCCESS and the attribute ID field omitted.
			writeRspCmd.attrList[0].status = ZCL_STATUS_SUCCESS;
    		writeRspCmd.numAttr = 1;
		}
